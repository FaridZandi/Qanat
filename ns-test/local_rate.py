...
local_storage_rates = range(100000, 1000000, 100000)
	...
	for topology_shape in topology_shapes: 
		for eventual_timeout in eventual_timeouts:
			for access_mode in access_modes:
				for local_storage_rate in local_storage_rates:
					i += 1 
					prio_scheme_ = 2 
					scheme = 'unknown'
					...
					directory_name = 'results_local_rate/%d_%s_%f_%i_%i_%i' % (int(load*100), access_mode_name, eventual_timeout, key_type, topology_shape, i)
