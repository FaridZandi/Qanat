...
int MigrationManagerTopDown::tunnel_uid_counter = 0;

MigrationManagerTopDown::MigrationManagerTopDown() : MigrationManager() {
...
MigrationManagerTopDown::~MigrationManagerTopDown() {
    delete[] tunnels;
}
...
int MigrationManagerTopDown::activate_tunnel(Node* in, Node* out, Node* from, Node* to) {
    tunnel_data td{};
    td.valid = true;
    td.in    = in;
    td.out   = out;
    td.from  = from;
    td.to    = to;
    td.uid   = MigrationManagerTopDown::tunnel_uid_counter++;

    add_tunnel(td);
    return td.uid;
}

void MigrationManagerTopDown::deactivate_tunnel(int uid) {
    for (int i = 0; i < MigrationManagerTopDown::tunnel_count; ++i) {
...
void MigrationManagerTopDown::add_tunnel(tunnel_data tunnel) {
    for (int i = 0; i < MigrationManagerTopDown::tunnel_count; ++i) {
        if (!tunnels[i].valid) {
            tunnels[i] = tunnel;
            active_tunnels++;
    ...
    std::cerr << "[MigrationManagerTopDown] Not enough space to store tunnels. Exiting!\n";
    std::exit(1);
}
...
void MigrationManagerTopDown::log_tunnel(tunnel_data td, Tunnel_Point tp, Packet* /*p*/) {
...
Tunnel_Point MigrationManagerTopDown::packet_match(tunnel_data td, Packet* p, Node* n) {
...
Direction MigrationManagerTopDown::packet_dir(tunnel_data /*td*/, Packet* /*p*/) {
...
bool MigrationManagerTopDown::pre_classify(Packet* p, Handler* h, Node* n) {
...
void MigrationManagerTopDown::handle_non_ready_nodes(Packet* p, Node* node) {
...
int MigrationManagerTopDown::get_packet_dst(Packet* p) {
    hdr_ip* iph = hdr_ip::access(p);
...
void MigrationManagerTopDown::set_packet_src(Packet* p, int src) {
    hdr_ip* iph = hdr_ip::access(p);
    iph->src_.addr_ = src;
}
...
bool MigrationManagerTopDown::tunnel_packet_in(tunnel_data td, Packet* p, Node* /*n*/) {
...
bool MigrationManagerTopDown::tunnel_packet_out(tunnel_data /*td*/, Packet* p, Node* n) {
...
bool MigrationManagerTopDown::handle_packet_from(tunnel_data td, Packet* p, Handler* h, Node* n) {
    ...
    // Should not happen
    if (verbose) {
        std::cout << "[MigrationManagerTopDown] Unexpected path in handle_packet_from at node "
                  << n->address() << std::endl;
    }
    return true;
}

bool MigrationManagerTopDown::handle_packet_to(tunnel_data td, Packet* p, Handler* h, Node* n) {
