...
int MigrationManagerTopDown::activate_tunnel(Node* in, Node* out, 
                                 	  Node* from, Node* to) {   
...
void MigrationManagerTopDown::log_tunnel(tunnel_data td, 
								  Tunnel_Point tp, 
								  Packet* p){
	if (not verbose){
		return; 
...
Tunnel_Point MigrationManagerTopDown::packet_match(tunnel_data td, 
											Packet* p, Node* n){
...
Direction MigrationManagerTopDown::packet_dir(tunnel_data td, 
									   Packet* p){
	// hdr_ip* iph = hdr_ip::access(p); 
...
bool MigrationManagerTopDown::pre_classify(Packet* p, Handler* h, Node* n){
    ...
    for(int i = 0; i < MigrationManagerTopDown::active_tunnels; i++){ 
		auto td = tunnels[i]; 
...
void MigrationManagerTopDown::handle_non_ready_nodes(Packet* p, Node* node){
	...
	if (topo.orch_type == 2 or topo.orch_type == 3) {
        ...
        if (topo.get_data(node).which_tree == 1){
			...
			// 	std::cout << "MigrationManagerTopDown::handle_non_ready_nodes: " << node->address() << std::endl; 
...
bool MigrationManagerTopDown::tunnel_packet_in(tunnel_data td, 
								  		Packet*p, Node* n){			  
...
bool MigrationManagerTopDown::tunnel_packet_out(tunnel_data td, 
								   Packet*p, Node* n){
...
bool MigrationManagerTopDown::handle_packet_from(tunnel_data td, 
                                      Packet*p, Handler* h, 
                                      Node* n){
	...
	if (dir == Direction::Outgoing) { 
		...
		if (node_state == MigState::Migrated or 
			...
			// std::cout << "MigrationManagerTopDown::handle_packet_from: " << n->address() << std::endl;
...
bool MigrationManagerTopDown::handle_packet_to(tunnel_data td, 
										Packet*p, 
										Handler* h,
										Node* n){
	hdr_ip* iph = hdr_ip::access(p); 	
