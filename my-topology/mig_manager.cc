...
int MigrationManager::activate_tunnel(Node* in, Node* out, Node* from, Node* to) {
    tunnel_data td{};
    td.valid = true;
    td.in    = in;
    td.out   = out;
...
void MigrationManager::add_tunnel(tunnel_data tunnel) {
    ...
    std::cerr << "[MigrationManager] Not enough space to store tunnels. Exiting!\n";
    std::exit(1);
}
...
void MigrationManager::log_tunnel(tunnel_data td, Tunnel_Point tp, Packet* /*p*/) {
...
Tunnel_Point MigrationManager::packet_match(tunnel_data td, Packet* p, Node* n) {
...
Direction MigrationManager::packet_dir(tunnel_data /*td*/, Packet* /*p*/) {
    return Direction::Dir_None;
}

// BUGFIX: original returned after checking only the first tunnel (and could
// return false even if a later tunnel matched). Now: scan all valid tunnels.
bool MigrationManager::bypass_processing(Packet* p, Handler* /*h*/, Node* n) {
    for (int i = 0; i < MigrationManager::tunnel_count; ++i) {
        if (!tunnels[i].valid) continue;
        if (packet_match(tunnels[i], p, n) != Tunnel_Point::Tunnel_None) {
            return true;
        }
    }
    return false;
...
bool MigrationManager::tunnel_packet_in(tunnel_data td, Packet* p, Node* /*n*/) {
...
bool MigrationManager::tunnel_packet_out(tunnel_data /*td*/, Packet* p, Node* n) {
...
bool MigrationManager::handle_packet_from(tunnel_data td, Packet* p, Handler* h, Node* n) {
    ...
    // Should not happen
    if (verbose) {
        std::cout << "[MigrationManager] Unexpected path in handle_packet_from at node "
                  << n->address() << std::endl;
    }
    return true;
}

bool MigrationManager::handle_packet_to(tunnel_data td, Packet* p, Handler* h, Node* n) {
